...
.then(function (connexion) {
    ...
    module.exports.getFromFieldWithout = function(tableName, id, field, toRemove) {
        return rdb.table(tableName).get(id).getField(field).difference(toRemove).run(connexion)
    }
    module.exports.deleteFromInterests = function(tableName, id, toDelete) {
        return rdb.table(tableName).get(id).update(function(row) {
