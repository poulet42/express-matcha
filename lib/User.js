...
.then(function (connexion) {
	...
	module.exports.toggleLike = function(liker, liked) {
		return rdb.table('likes').contains(rdb.row('emitter').eq('poulet'))
		.branch(
	...
	module.exports.notify = function(text, receiver) {
		return rdb.table('notifications').insert({text, receiver}).run(connexion)
	}
})